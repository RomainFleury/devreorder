...
DeviceLister can also manage and generate `devreorder.ini` files for you. Choose a selected game/program folder, the all-programs `C:\ProgramData\devreorder\devreorder.ini` target, or the system-wide install target, then use **Load Target** or **Save to Target**. For a selected game, **Pick EXE...** detects whether the executable is x86 or x64, **Install for Game** copies the matching wrapper DLL and saves the config, and **Uninstall from Game** removes only a known devreorder DLL copy while restoring a backup if one was made. Use **Import...** to load an existing `devreorder.ini` from another folder without changing the selected target, or save/load named profiles from DeviceLister's per-user settings. Check the controllers that should remain visible, uncheck controllers that should be hidden, drag the rows or use the move buttons to set the order, and optionally edit the ignored-process list. The preview panel shows the effective visible order that games should see after hidden devices are removed. Selecting a device starts a live input helper so you can press buttons or move axes to identify which physical controller it is; this is especially useful for duplicate controller names. You can also assign a friendly alias to a device. Aliases are stored in DeviceLister's per-user settings instead of `devreorder.ini`, and do not change the DirectInput product name seen by games. By default DeviceLister writes device instance IDs when available, falling back to GUIDs if needed; the drop-down can be changed to write GUIDs or product names instead.
